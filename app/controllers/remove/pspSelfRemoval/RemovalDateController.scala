...
class RemovalDateController @Inject()(override val messagesApi: MessagesApi,
  ...
  def onPageLoad: Action[AnyContent] = (authenticate(PSP) andThen getData andThen requireData).async {
    implicit request =>
      (SchemeNameId and SchemeSrnId and AssociatedDateId).retrieve.right.map {
        case schemeName ~ srn ~ associationDate =>
          val preparedForm = request.userAnswers.get(RemovalDateId).fold(form(associationDate))(form(associationDate).fill)
          Future.successful(Ok(view(preparedForm, schemeName, srn, formatDate(associationDate))))
      }
  }
        ...
        case schemeName ~ srn ~ associationDate =>
          form(associationDate).bindFromRequest().fold(
            (formWithErrors: Form[_]) =>
              Future.successful(BadRequest(view(formWithErrors, schemeName, srn, formatDate(associationDate)))),
            value =>
