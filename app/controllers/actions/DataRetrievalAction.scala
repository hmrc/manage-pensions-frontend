...
class DataRetrievalActionImpl @Inject()(val dataCacheConnector: UserAnswersCacheConnector) extends DataRetrievalAction {
  ...
  override protected def transform[A](request: AuthenticatedRequest[A]): Future[OptionalDataRequest[A]] = {
    ...
    dataCacheConnector.fetch(request.externalId).map {
      case None => OptionalDataRequest(request.request, request.externalId, None, request.psaId, request.userId)
      case Some(data) => OptionalDataRequest(request.request, request.externalId, Some(new UserAnswers(data)), request.psaId, request.userId)
    }
  }
}
