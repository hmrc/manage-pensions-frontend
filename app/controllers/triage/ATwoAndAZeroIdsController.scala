...
class ATwoAndAZeroIdsController @Inject()(
                                           override val messagesApi: MessagesApi,
                                           frontendAppConfig: FrontendAppConfig,
                                           triageAction: TriageAction,
                                           val controllerComponents: MessagesControllerComponents,
                                           view: aTwoAndAZeroIds
  ...
  def onPageLoad(): Action[AnyContent] = triageAction.async {
    implicit request =>
      ...
      val returnLink = frontendAppConfig.guidanceStartPageGovUkLink

      Future.successful(
        Ok(view(managePensionSchemesServiceLink, pensionSchemesOnlineServiceLink, returnLink)))
  }
}
