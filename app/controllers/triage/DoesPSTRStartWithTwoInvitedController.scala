  ...
  def onSubmit: Action[AnyContent] = triageAction.async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          val uaUpdated = UserAnswers().set(DoesPSTRStartWithTwoInvitedId)(value).asOpt.getOrElse(UserAnswers())
          Future.successful(Redirect(navigator.nextPage(DoesPSTRStartWithTwoInvitedId, NormalMode, uaUpdated)(request, implicitly, implicitly)))
        }
      )
  }
}
