...
import identifiers.triage.DoesPSTRStartWithTwoAuthPspId
...
class DoesPSTRStartWithTwoAuthPspController @Inject()(
  ...
  def onSubmit: Action[AnyContent] = triageAction.async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          val uaUpdated = UserAnswers().set(DoesPSTRStartWithTwoAuthPspId)(value).asOpt.getOrElse(UserAnswers())
          Future.successful(Redirect(navigator.nextPage(DoesPSTRStartWithTwoAuthPspId, NormalMode, uaUpdated)))
        }
      )
  }
}
