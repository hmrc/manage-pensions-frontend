...
import identifiers.triage.{DoesPSTRStartWithTwoDeAuthId, DoesPSTRStartWithTwoInvitedId}
...
class DoesPSTRStartWithTwoDeAuthController @Inject()(
  ...
  def onSubmit: Action[AnyContent] = triageAction.async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          val uaUpdated = UserAnswers().set(DoesPSTRStartWithTwoDeAuthId)(value).asOpt.getOrElse(UserAnswers())
          Future.successful(Redirect(navigator.nextPage(DoesPSTRStartWithTwoDeAuthId, NormalMode, uaUpdated)))
        }
      )
  }
}
