...
class FileEventReportController @Inject()(override val messagesApi: MessagesApi,
                                          val appConfig: FrontendAppConfig,
                                          triageAction: TriageAction,
                                          val controllerComponents: MessagesControllerComponents,
                                          val view: fileEventReport
  ...
  def onPageLoad(role: String): Action[AnyContent] = triageAction.async {
    implicit request =>
      ...
      Future.successful(Ok(view(role, pensionSchemesOnlineLink,  appConfig.submitEventReportGovUkLink)))
  }
}
