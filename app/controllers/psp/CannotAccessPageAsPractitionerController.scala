...
import controllers.psp.routes.PspDashboardController
...
class CannotAccessPageAsPractitionerController @Inject()(
                                                         ...
                                                         view: cannotAccessPageAsPractitioner
  ...
  def onPageLoad: Action[AnyContent] = auth().async {
    implicit request =>
      request.request.queryString.find(_._1 == "continue").flatMap(_._2.headOption) match {
        ...
        case _ => Future.successful(Redirect(PspDashboardController.onPageLoad()))
      }
  }
        ...
        value => {
          cacheConnector.fetch(request.externalId).flatMap { optionJsValue =>
            ...
            val (finalOptionUA, finalResult) = (value, optionContinueUrl) match {
              case (Practitioner, _) =>
                (optionUAContinueURLRemoved, Redirect(PspDashboardController.onPageLoad()))
