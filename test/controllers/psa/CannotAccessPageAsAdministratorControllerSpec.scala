...
class CannotAccessPageAsAdministratorControllerSpec
  ...
  "CannotAccessPageAsAdministratorController" must {
    ...
    "return OK with the view when calling on page load with continue parameter" in {
      val request = addCSRFToken(FakeRequest(GET, s"${CannotAccessPageAsAdministratorController.onPageLoad().url}?continue=/test" ))
    ...
    "redirect when calling on page load with no continue parameter" in {
      val request = addCSRFToken(FakeRequest(GET, CannotAccessPageAsAdministratorController.onPageLoad().url))
      val result = controller.onPageLoad(request)

      status(result) mustBe SEE_OTHER
      redirectLocation(result) mustBe Some(SchemesOverviewController.onPageLoad().url)
    }
