...
class InvitingPSTRStartWithTwoControllerSpec extends ControllerSpecBase with ScalaFutures with MockitoSugar {
  ...
  "InvitingPSTRStartWithTwoController" must {
    ...
    "return OK with the view when calling on page load" in {
      val request = addCSRFToken(FakeRequest(GET, routes.InvitingPSTRStartWithTwoController.onPageLoad().url))
