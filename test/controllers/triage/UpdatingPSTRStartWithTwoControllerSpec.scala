...
class UpdatingPSTRStartWithTwoControllerSpec extends ControllerSpecBase with ScalaFutures with MockitoSugar {

  private val application = applicationBuilder().build()

  private val view = injector.instanceOf[updatingPSTRStartWithTwo]

  "UpdatingPSTRStartWithTwoController" must {

    "return OK with the view when calling on page load" in {
      val request = addCSRFToken(FakeRequest(GET, routes.UpdatingPSTRStartWithTwoController.onPageLoad().url))
