...
class DoesPSTRStartWithTwoInviteControllerSpec extends ControllerSpecBase with ScalaFutures with MockitoSugar {
  ...
  "WhatDoYouWantToDoController" must {
    ...
    "return OK with the view when calling on page load" in {
      val request = addCSRFToken(FakeRequest(GET, routes.DoesPSTRStartWithTwoInviteController.onPageLoad().url))
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      val postRequest = FakeRequest(POST, routes.DoesPSTRStartWithTwoInviteController.onSubmit().url).withFormUrlEncodedBody("value" -> "invalid value")
    ...
    "redirect to the next page for a valid request" in {
      val postRequest = FakeRequest(POST, routes.DoesPSTRStartWithTwoInviteController.onSubmit().url).withFormUrlEncodedBody("value" -> "true")
      val result = route(application, postRequest).value
