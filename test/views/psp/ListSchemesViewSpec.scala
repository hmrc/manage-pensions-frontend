  ...
  "list-schemes view" must {
    ...
    "show correct pagination links when number of schemes is greater than pagination at start of range" in {
      ...
      pageNumberLinks.foreach { index =>
        assertLink(actual, s"pageNumber-$index", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(index).url)
      }

      assertNotRenderedByCssSelector(actual, "#pageNumber-6")
      assertLink(actual, "next", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(2).url)
      assertLink(actual, "last", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(numberOfPages).url)
    ...
    "show correct pagination links when number of schemes is greater than pagination at middle of range" in {
      ...
      assertLink(actual, "first", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(1).url)
      assertLink(actual, "prev", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(3).url)

      pageNumberLinks.foreach { index =>
        assertLink(actual, s"pageNumber-$index", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(index).url)
      }

      assertLink(actual, "next", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(5).url)
      assertLink(actual, "last", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(numberOfPages).url)
      assertEqualsValue(actual, "#next", messages("messages__schemesOverview__pagination__next"))
      assertEqualsValue(actual, "#last", messages("messages__schemesOverview__pagination__last"))
    }
    ...
    "show correct pagination links when number of schemes is greater than pagination at end of range" in {
      ...
      assertEqualsValue(actual, "#prev", messages("messages__schemesOverview__pagination__prev"))
      assertLink(actual, "first", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(1).url)
      assertLink(actual, "prev", controllers.psp.routes.ListSchemesController.onPageLoadWithPageNumber(10).url)
