...
class ConfirmDeauthViewSpec extends YesNoViewBehaviours {
  ...
  private val confirmDeauthView = injector.instanceOf[confirmDeauth]

  private def createView: () => HtmlFormat.Appendable = () =>
    confirmDeauthView(form, schemeName, srn, pspName)(fakeRequest, messages)

  private def createViewUsingForm: Form[_] => HtmlFormat.Appendable = (form: Form[_]) =>
    confirmDeauthView(form, schemeName, srn, pspName)(fakeRequest, messages)

  "ConfirmDeauthorise" must {
    ...
    behave like yesNoPage(
      ...
      messageKeyPrefix = prefix,
      expectedFormAction = routes.ConfirmDeauthController.onSubmit().url
    )
